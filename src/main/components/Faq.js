	...
	state = {
		topic: [
			...
			},
			{
				...
				text: "Anyone can participate, as long as you have an internet connection (to use our website) and access to a bitcoin wallet (to deposit and Withdraw your money). Also, please note we don’t accept people from the USA and Canada."
			},
		],
    	openedAll: false, 
	}
	...
	render () {
		...
		return (
			<div className="faq-topic">
				{ this.state.topic && this.state.topic.map(item => (
					<div key={item.id} className={"faq-topic-" + item.id + "-" + (item.opened ? "-opened" : "") }>
				        <div className="faq-topic-question-opened">
				        	<p>{item.question}</p>
				        </div>
				        <div onClick={() => this.toggleTopic(item.id)} className={ "faq-topic-arrow" + (item.opened ? "-opened" : "") }/>
				        <div className={"faq-topic-answer" + (item.opened ? "-opened" : "") }>
				            <p>{item.text}</p>
				        </div>
				    </div>	
				))}
		    	<div className="faq-topic-button">
		        	<button onClick={() => this.toggleAll()}>{ !this.state.openedAll ? "View All" : "Close All" }</button>
		      	</div>
		    </div>
		)
	}
