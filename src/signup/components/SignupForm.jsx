...
const asyncValidate = (values/*, dispatch*/) => {
	return checkUserGetFetch({ Username: values.username, Email: values.email })
	.then(res => {
		if (res.ok) {
			return undefined
...
const textField = ({ input, placeholder, className, type, meta: { asyncValidating, touched, error, warning }}) => {
	return (
...
const checkField = ({ input, className, type, id, link, linktext, text, meta: { touched, error }}) => {
	return (
	    <div className={"signup-box-" + className}>
	        <label className={"signup-form-" + className} htmlFor={id}>
		        ...
		        {...input}
			        ...
			        id={id}
		        />
                <span className={touched && error ? "checkmark-" + className + "-error" : "checkmark-" + className}></span>
...
class SignupForm extends Component {
	...
	render() {
		const { handleSubmit, reset, pristine, submitting, createUser, error } = this.props
		...
		if (createUser.error.type === 'done' ) {
			return(
				<section className="signup-wrapper">
					<form className="signup-box">
						<div className="signup-box-header">
							<h2>Thanks for creating your defima account, please go to your email inbox and confirm your registration</h2>
						</div>
					</form>
				</section>
			)
			...
			return(
				<section className="signup-wrapper">
					{(createUser.isFetching || submitting) && <Loader type="Rings" color="#ffffff" height={100} width={100}/>}
				    <form
				    	className={createUser.isFetching || submitting ? "none" : "signup-box"}
