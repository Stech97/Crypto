...
const DashInfoFetch = async (type, period) => {
	let response = await API("/Administrator/" + type + "?Period=" + period);
	return response;
};
...
const DashInfoAction = (type, period, dispatch) => {
	dispatch(DashInfoRequest(type));
	DashInfoFetch(type, period)
		.then((res) => {
			if (res.ok) {
...
export const getDashInfo = (period) => {
	return (dispatch) => {
		let actions = [
			GetAddedFounds,
			GetInvestedAmount,
		...
		for (const type of actions) {
			DashInfoAction(type, period, dispatch);
		}
	};
};
